...
namespace FacebookLoginTesting.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl)
        {
            ...
            cookingapp_latestEntities db = new cookingapp_latestEntities();
            if (result == SignInStatus.Success)
            {   
                user_list user = db.user_list.Where(x => x.user_email == email).FirstOrDefault();
                if (user == null)
                {
                    user_list new_user = new user_list();
                    new_user.user_email = email;
                    new_user.username = name;
                    db.user_list.Add(new_user);
                    db.SaveChanges();
                }
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        cookingapp_latestEntities db = new cookingapp_latestEntities();
                        user_list new_user = new user_list();
                        new_user.user_email = userInfo["email"];
                        new_user.username = userInfo["first_name"] + " " + userInfo["last_name"];
                        db.user_list.Add(new_user);
                        db.SaveChanges();
